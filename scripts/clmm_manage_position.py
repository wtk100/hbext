############################################################################################################################
### GatewayHttpClient使用的简单示例，Takeaways:
### 1. 如何检查Gateway状态及通过GatewayHttpClient获取定价信息
### 2. 如何根据价格带配置开平仓、监控仓位
### CLMM=Concentrated Liquidity Market Maker，与AMM一样是去中心化交易机制；CLMM的核心在于“集中流动性”（Concentrated Liquidity），
### 即流动性提供者可以选择一个特定的价格区间，将流动性集中在该区间内，而不是像传统AMM（如Uniswap V2）那样将流动性均匀分布在所有价格区间
############################################################################################################################
import asyncio
import os
import time
